...
void loop() {
  if (Serial.available() > 0) {
    ...
    desPos1 = (command.substring(0, commaLoc1+1)).toFloat();
    desPos2 = desPos1;
    if (commaLoc1 != -1) desPos2 = (command.substring(commaLoc1+1, commaLoc2)).toFloat();
    if (!motor1Running) {
      theta1 = desPos1 - readEncoder(curPos1, A1);
      theta2 = desPos2 - readEncoder(curPos2, A2);
      theta3 = desPos3 - readEncoder(curPos3, A3);
      theta4 = desPos4 - readEncoder(curPos4, A4);
